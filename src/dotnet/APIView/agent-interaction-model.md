...
- **User says:** "Open the package azure-core" / "Tell me about package {packageName}"
- **Agent action:** Fetch package metadata by ID or resolve from package name + language.
...
- **Impact:** HIGH
- **Implemented:** 🟡 `GET /api/reviews/resolve` (token-auth) can resolve a package from package name + language + version or URL. `GET /api/reviews/{reviewId}` is cookie-auth only. Not in CLI/MCP as a standalone command.
...
- **Impact:** HIGH
- **Implemented:** ❌ `POST /api/apirevisions/{reviewId}/generateReview` is cookie-auth only and returns a job ID for polling. No streaming support exists. Not in CLI/MCP.
...
- **Impact:** MEDIUM
- **Implemented:** ❌ No token-auth endpoint exists for AI overview generation. The backend would need to expose a token-auth endpoint that proxies to AVC.
- **Alternative:** The agent could skip the AVC proxy and instead fetch the API surface text (3c) and generate its own summary using the host LLM (e.g., GitHub Copilot). This avoids the need for a new backend endpoint but loses the benefit of AVC's domain-tuned prompts and caching.
...
- **Impact:** MEDIUM
- **Implemented:** ❌ The Copilot diff-summary endpoint is cookie-auth only. Not in CLI/MCP.
- **Alternative:** The agent could fetch the raw diff via the diff endpoint (4a) and generate its own summary using the host LLM (e.g., GitHub Copilot). This avoids dependency on the Copilot diff-summary endpoint but loses the benefit of AVC's domain-tuned prompts and caching.
